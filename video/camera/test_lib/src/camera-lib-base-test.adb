...
-- use type Index_Type;
   ...
   type Test_Type (
      Brand : Brand_Type) is new Camera.Lib.Unit_Test.Camera_Test_Type (Brand)
         with null record;
...
-- procedure Port_Scan( move to Ada_Lib.Socket_IO.Client.Test
...
--    First_Port                 : constant GNAT.Sockets.Port_Type := 8998;   -- 1
--    Last_Port                  : constant GNAT.Sockets.Port_Type := 9001; -- 9999;
--    Local_Test                 : Test_Type renames Test_Type (Test);
--    Opened_Ports               : Natural := 0;
--
-- begin
--    Put_Line ("test port scan on " & Options.Camera_Options.Camera_Address.Image);
--    for Port in First_Port .. Last_Port loop
--       begin
--          Local_Test.Camera_Queue.Open (
--             Options.Camera_Options.Camera_Address.all, Port,
...
--
--       Log_Here (Debug);
--       Local_Test.Camera_Queue.Close;
--    end loop;
--    Put_Line ("port scan completed. Opened ports" & Opened_Ports'img);
--
-- exception
--    when Fault: others =>
--       Trace_Exception (Debug, Fault);
...
--            begin
--               Log_In (Debug);
--               Local_Test.Camera_Queue.Send_Command (
...
--                                    Local_Test.Camera_Queue.Get_Ack_Length;
--                  begin
--                     Local_Test.Camera_Queue.Get_Response (Get_Ack, Has_Response,
--                        Buffer, Response_Length,
--                        Local_Test.Camera_Queue.Get_Timeout (Memory_Recall));
--
--                     Local_Test.Camera_Queue.Acked (Buffer, Ack_Value, Next_Byte);
--                     Put_Line ("ack value" & Hex_IO.Hex (Ack_Value, 8) & " next value" & Next_Byte'img);
--                     if Next_Byte > 0 then
--                        Local_Test.Camera_Queue.Completed (
...
--            Local_Test.Camera_Queue.URL_Open (
...
--   end Read_Write;
   ...
   begin
      ...
      Test.Add_Routine (AUnit.Test_Cases.Routine_Spec'(
         Routine        => Test_Reopen'access,
         Routine_Name   => AUnit.Format ("Test_Reopen")));
...
-- end Tear_Down;
   ...
   begin
      Log_In (Debug);
      Put_Line ("test open");
      Assert (Local_Test.Camera_Queue /= Null, "null queue");
      Assert (Options.Camera_Options.Camera_Address /= Null, "null address");
      Local_Test.Camera_Queue.Open (Options.Camera_Options.Camera_Address.all,
         Options.Camera_Options.Port_Number);
      Log_Out (Debug);
   ...
   begin
      Put_Line ("test reopen");
      Log_In (Debug);
      Local_Test.Camera_Queue.Open (Options.Camera_Options.Camera_Address.all,
         Options.Camera_Options.Port_Number);
      Log_Here (Debug);
      Local_Test.Camera_Queue.Reopen;
      Log_Out (Debug);
...
----       IP_Address (4) := Unit;
--         begin
--            Local_Test.Camera_Queue.Open (Options.Camera_Options.Camera_Address.all, 80);     -- use default HTTP port
